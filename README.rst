...
The publish workflow also emits ``-supabase`` and ``-supabase-auto-init`` tag variants for each published tag. Both variants keep the same image contents and default ``ENABLE_SUPABASE_EXTENSIONS=true`` in the image metadata. The ``-supabase`` variant leaves ``ENABLE_SUPABASE_INIT=false`` so database bootstrap remains an explicit opt-in. The ``-supabase-auto-init`` variant sets ``ENABLE_SUPABASE_INIT=true`` and matches the auto-initializing behavior of ``supabase/postgres``: if no external pgsodium root key source is provided, Spilo generates a persistent cluster-local key file inside ``PGDATA`` so Supabase startup and bootstrap can still complete. Explicit ``PGSODIUM_KEY``, ``PGSODIUM_KEY_FILE``, or custom ``pgsodium.getkey_script`` settings override that fallback.
