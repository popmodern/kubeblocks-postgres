...
- **ENABLE_SUPABASE_INIT**: run the optional Supabase bootstrap during post-init. PostgreSQL 15 and newer use a pinned copy of the upstream ``supabase/postgres`` SQL migration bundle against the ``postgres`` database. PostgreSQL 14 still needs a local compatibility fallback because upstream no longer ships a PG14 bootstrap artifact we can vendor directly. This requires ``ENABLE_SUPABASE_EXTENSIONS=true`` because the bootstrap assumes those extensions are intentionally enabled. The bundled bootstrap also needs a pgsodium root key source, either via ``PGSODIUM_KEY``, ``PGSODIUM_KEY_FILE``, or a custom ``postgresql.parameters.pgsodium.getkey_script``. If bootstrap is requested without that key configuration, Spilo keeps PostgreSQL running, logs that Supabase bootstrap was skipped, and lets you rerun ``/scripts/run_supabase_migrations.sh <database>`` after the key is available.
...
- **PGSODIUM_KEY**: hex-encoded 32 byte pgsodium root key provided from a secret-backed environment variable. Use this only when you cannot mount a secret file; ``PGSODIUM_KEY_FILE`` is the preferred path because it keeps the root key out of the process environment. This key is required for the default server-managed pgsodium key path and for the bundled Supabase bootstrap when you rely on Spilo's automatic ``pgsodium.getkey_script`` wiring.
- **PGSODIUM_KEY_FILE**: path to a mounted secret file containing the hex-encoded 32 byte pgsodium root key. This is the preferred way to provide the pgsodium root key.
