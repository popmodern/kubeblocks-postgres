...
- **ENABLE_SUPABASE_EXTENSIONS**: opt in to Supabase-specific extension runtime configuration. When set to ``true``, Spilo adds the Supabase preload libraries and extends the extension whitelist for the Supabase-related extensions that are built into the image. The default is ``false`` so these higher-risk surfaces are not enabled automatically. The published ``-supabase`` image variant flips this default to ``true`` in the image metadata.
- **ENABLE_SUPABASE_INIT**: run the optional Supabase bootstrap during post-init. PostgreSQL 15 and newer use a pinned copy of the upstream ``supabase/postgres`` SQL migration bundle against the ``postgres`` database. PostgreSQL 14 still needs a local compatibility fallback because upstream no longer ships a PG14 bootstrap artifact we can vendor directly. This requires ``ENABLE_SUPABASE_EXTENSIONS=true`` because the bootstrap assumes those extensions are intentionally enabled. Supabase-capable images always wire a working getkey script for ``pgsodium`` and ``vault`` at startup. If neither ``PGSODIUM_KEY`` nor ``PGSODIUM_KEY_FILE`` is provided, Spilo generates a persistent cluster-local pgsodium root key file inside ``PGDATA`` so Supabase startup and bootstrap still match ``supabase/postgres``. Provide an explicit key source or custom ``postgresql.parameters.pgsodium.getkey_script`` when you need key lifecycle control outside the cluster data directory.
- **SUPABASE_CUSTOM_MIGRATIONS_DIR**: optional directory of extra ``.sql`` files to run in lexical order after the bundled Supabase migrations. Defaults to ``/etc/postgresql.schema.d``. PostgreSQL 14 applies this after the local compatibility bootstrap in each database handled by that fallback.
- **SUPABASE_CUSTOM_MIGRATION_FILE**: optional final SQL file to run after the bundled Supabase migrations and any files from ``SUPABASE_CUSTOM_MIGRATIONS_DIR``. Defaults to ``/etc/postgresql.schema.sql`` to match upstream Supabase's post-migration hook. PostgreSQL 14 applies this after the local compatibility bootstrap in each database handled by that fallback.
- **PGSODIUM_KEY**: hex-encoded 32 byte pgsodium root key provided from a secret-backed environment variable. Use this only when you cannot mount a secret file; ``PGSODIUM_KEY_FILE`` is the preferred path because it keeps the root key out of the process environment. When set, this overrides Spilo's generated fallback key and is materialized into a runtime file before Patroni starts.
- **PGSODIUM_KEY_FILE**: path to a mounted secret file containing the hex-encoded 32 byte pgsodium root key. This is the preferred way to provide the pgsodium root key. When unset and Supabase runtime support is enabled, Spilo falls back to ``$PGDATA/pgsodium_root.key`` and generates the file on first boot so ``pgsodium`` and ``vault`` can start automatically.
