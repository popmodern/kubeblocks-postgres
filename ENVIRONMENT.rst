...
- **ENABLE_SUPABASE_INIT**: run the optional Supabase bootstrap during post-init. PostgreSQL 15 and newer use a pinned copy of the upstream ``supabase/postgres`` SQL migration bundle against the ``postgres`` database, while PostgreSQL 14 falls back to the local legacy compatibility SQL. This requires ``ENABLE_SUPABASE_EXTENSIONS=true`` because the bootstrap assumes those extensions are intentionally enabled.
